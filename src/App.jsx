...
import SenateHall from './components/SenateHall.jsx';

function GameUI() {
  const {
    meters, currentCard, objectives, completedObjectives, turn, swipe,
    activeRoom, pendingVote, enterRoom,
  ...
  return (
    <div className="flex flex-col h-full w-full">
      {/* Faction meters — top bar, shared across all rooms */}
      <div className="flex-shrink-0 border-b border-border/30 bg-void/80 backdrop-blur-sm">
        <FactionMeters meters={meters} />
      </div>

      {activeRoom === 'office' && <Office />}

      {activeRoom === 'vote' && <SenateHall />}

      {activeRoom === 'audience' && (
        <>
          {/* Office link + turn counter */}
          <div className="flex-shrink-0 flex items-center justify-between px-4 py-1">
            <button
              onClick={() => enterRoom('office')}
              className="text-[10px] font-display text-soft/40 hover:text-gold uppercase tracking-widest transition-colors"
            >
              ← Office
            </button>
            <span className="text-[10px] font-display text-soft/25 uppercase tracking-widest">
              {pendingVote ? 'Senate in session' : `Turn ${turn}`}
            </span>
          </div>
          ...
          <div className="flex-shrink-0">
            <ObjectiveHints objectives={objectives} completedObjectives={completedObjectives} />
          </div>
        </>
      )}
    </div>
